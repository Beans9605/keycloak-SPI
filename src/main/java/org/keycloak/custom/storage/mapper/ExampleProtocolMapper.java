...
import org.keycloak.custom.storage.dto.ClientRole;
...
public class ExampleProtocolMapper extends AbstractOIDCProtocolMapper
        implements UserInfoTokenMapper, OIDCAccessTokenMapper, OIDCIDTokenMapper {

    private final Logger logger = Logger.getLogger(ExampleProtocolMapper.class);

    public static final String PROVIDER_ID = "example-protocol-mapper";
    public static final List<ProviderConfigProperty> configProperties = new ArrayList<>();

    static {
        OIDCAttributeMapperHelper.addTokenClaimNameConfig(configProperties);
        OIDCAttributeMapperHelper.addIncludeInTokensConfig(configProperties, ExampleProtocolMapper.class);
    }
     ...
     *     따라서 이 값을 추출하여, userEntity 가 가진 sideClient 들의 role 에 대한 정보를 합쳐 인가 정보를 쓰고 싶은 사용자들이 자유롭게 사용할 수 있도록 함
     * </p>
     *
     ...
     */
    @Transactional
    protected List<ClientRole> returnClientRoles(KeycloakSession session, UserSessionModel userSession) {
        ...
        if (userId != null) {
            ...
            return uscs.stream().map(ClientRole::from).toList();
        }
    ...
    protected void setClaim(AccessTokenResponse token, ProtocolMapperModel mappingModel,
        ...
        OIDCAttributeMapperHelper.mapClaim(token, mappingModel, returnClientRoles(keycloakSession, userSession));
    }
